...
### ⚠️ MCP 返回的 ID 与原始 API 的 kb_id 不一致（写操作必读）

- **现象**：`mcp__ima-mcp__get_knowledge_base_list` 返回的 `id` 形如 `7481xxxxxxxxxxxx`（数字串），但 `ima_api.cjs` 直连原始 OpenAPI 的写接口（`add_knowledge` / `import_urls` / `get_knowledge_base` / `get_knowledge_list`）用这些数字 ID 会返回 `220004 invalid knowledge_base_id`。MCP 读取（`get_knowledge_list` 等）却正常——两层的 ID 空间不同。
- **正确做法**：写操作前，先调用原始 API `openapi/wiki/v1/search_knowledge_base` 传 `{"query":"","cursor":"","limit":20}`，从 `data.info_list[].kb_id` 取**真实可写 ID**（形如 `W7iiDUL1CqeiDnYtkYohWu-ilDzcl6sPRwRtxMOJrHg=`），用于所有写接口。
- **import_urls 落到根目录**：`folder_id` 必须**省略**；传 kb_id 当 folder_id 会报 `222000 文件夹不存在`。
- **删除 / 重命名 / 新建空文件夹 / 导出备份**：原始 API 与当前 skill 均未提供受支持的接口，只能到 ima 桌面端手动操作。
